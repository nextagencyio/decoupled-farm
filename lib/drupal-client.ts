            ...
            route(path: $path) {
              ... on RouteInternal {
                entity {
                  ...
                  ... on NodeProduct { __typename id title path body { processed } productCategory { ... on TermProductCategory { name } } price unit available growingMethod image { url alt width height } featured }
                  ... on NodeSeason { __typename id title path body { processed } startMonth endMonth whatsGrowing farmActivities image { url alt width height } }
                  ... on NodeRecipe { __typename id title path body { processed } prepTime cookTime servings ingredients recipeCategory { ... on TermRecipeCategory { name } } image { url alt width height } }
