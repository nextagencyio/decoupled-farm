...
import seasonsData from '@/data/mock/seasons.json'
import recipesData from '@/data/mock/recipes.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetProducts') || query.includes('nodeProducts')) {
      return loadMockData('products.json')
    }

    if (query.includes('GetSeasons') || query.includes('nodeSeasons')) {
      return loadMockData('seasons.json')
    }

    if (query.includes('GetRecipes') || query.includes('nodeRecipes')) {
      return loadMockData('recipes.json')
    }
