...
export default function SeasonCard({ item }: SeasonCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      <div className="relative h-52 bg-gradient-to-br from-primary-100 to-accent-100">
      ...
      <div className="p-6">
        {(item as any).startMonth && (
          <p className="text-sm text-primary-700 font-semibold mb-2">
            {(item as any).startMonth}{(item as any).endMonth ? ` - ${(item as any).endMonth}` : ''}
          </p>
        )}
