...
import { GET_SEASON_BY_PATH } from '@/lib/queries'
import { DrupalSeason } from '@/lib/types'
...
interface SeasonByPathData {
  route: {
    entity: DrupalSeason
  } | null
}

async function getSeason(path: string): Promise<DrupalSeason | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<SeasonByPathData>({
      query: GET_SEASON_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/seasons/${slug.join('/')}`
  const item = await getSeason(path)

  if (!item) {
    return { title: 'Season Not Found | Farm' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).startMonth && (
                    <div>
                      <dt className="text-sm text-gray-500">Start Month</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).startMonth}</dd>
                    </div>
                  )}
                  {(item as any).endMonth && (
                    <div>
                      <dt className="text-sm text-gray-500">End Month</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).endMonth}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
