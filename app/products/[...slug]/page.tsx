...
import { GET_PRODUCT_BY_PATH } from '@/lib/queries'
import { DrupalProduct } from '@/lib/types'
...
interface ProductByPathData {
  route: {
    entity: DrupalProduct
  } | null
}

async function getProduct(path: string): Promise<DrupalProduct | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ProductByPathData>({
      query: GET_PRODUCT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getProduct(path)

  if (!item) {
    return { title: 'Product Not Found | Farm' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).price && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).price}</dd>
                    </div>
                  )}
                  {(item as any).unit && (
                    <div>
                      <dt className="text-sm text-gray-500">Unit</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).unit}</dd>
                    </div>
                  )}
                  {(item as any).growingMethod && (
                    <div>
                      <dt className="text-sm text-gray-500">Growing Method</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).growingMethod}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
